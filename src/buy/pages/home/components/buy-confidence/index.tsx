...
export default function BuyConfidence(props: any) {
  ...
  const tabList = [
    ...
    {
      name: 'Craig',
      comments: 'Fast, professional, great price. Definitely doing business here in the future.',
      star: 5
    },
    {
      name: 'Jessica',
            ...
            tabList.map((t, index) => {
              return (
                <div className={"buy-home-confidence-mob"} key={index}>
                  <div className={"buy-confidence-comments"} dangerouslySetInnerHTML={{__html: t.comments}}/>
  ...
  return (
    <div className={"buy-home-confidence-web-wrapper"}>
      <div className={"buy-home-confidence-web"}>
        <div className={"buy-home-confidence-title"}>Buy With Confidence</div>
        <div className={"buy-confidence-comments"} dangerouslySetInnerHTML={{__html: chooseData.comments}}/>
          ...
          <div className={"tab-wrapper-overflow"}>
            {
              tabList.map((t, index) => {
                return (
                  <span key={index}
                        className={`tab-item ${chooseData.name === t.name ? "active" : "inactive"}`}
                        onClick={() => {
                          setChooseData(t)
                        }}>
                    {t.name}
