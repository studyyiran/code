...
import { EPayType, INoteUserModalProps } from "../index.interface";
...
export const brandIdValidate = (): boolean => {
  return !!(
    typeof yourPhoneStore.activeBrandsId === "number" &&
    yourPhoneStore.activeBrandsId !== -1
  );
};
...
export const inquiryDetail = (): boolean => {
  return !!(
    yourPhoneStore.inquiryDetail !== null &&
    JSON.stringify(yourPhoneStore.inquiryDetail) !== "{}"
  );
};

export const addressInfoValidate = (): boolean => {
  return !!(
    yourPhoneStore.addressInfo !== null &&
    JSON.stringify(yourPhoneStore.addressInfo) !== "{}" &&
    yourPhoneStore.addressInfo.state !== ""
  );
};

export const paymentValidate = (): boolean => {
  return !!(
    yourPhoneStore.payment &&
    (yourPhoneStore.payment === EPayType.ECHECK ||
      yourPhoneStore.payment === EPayType.PAYPAL)
  );
};
...
export const orderDetailValidate = (): boolean => {
  ...
  return !!(
    yourPhoneStore.inquiryDetail !== null &&
    JSON.stringify(yourPhoneStore.orderDetail) !== "{}"
  );
};
