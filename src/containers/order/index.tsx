...
class Order extends React.Component<IOrderProps & RouteComponentProps> {
  ...
  public render() {
    ...
    if (
      this.props.order.orderDetail.status === IProgressType.TO_BE_RECEIVED ||
      this.props.order.orderDetail.status === IProgressType.TO_BE_INSPECTED
    ) {
    ...
    // 是否展示质检模块
    if (
      this.props.order.orderDetail.status === IProgressType.DIFFERENCE_INSPECTED
    ) {
    ...
    // 是否退货发货
    if (
      this.props.order.orderDetail.status === IProgressType.TRANSACTION_FAILED
    ) {
    ...
    return (
      <div className="page-order-container">
        <section>
          ...
          {this.props.order.orderDetail.status ===
            IProgressType.TRANSACTION_SUCCEED && (
            <div className="reviews-wrapper">
              <h3>
                Thank you for using UpTrade, your review is important to us.
              </h3>
              <a
          ...
          <div className="page-order-body">
            ...
            {alreadyLoadData && (
              <OrderSummary
                ...
                normal={ReactNodeConfig.orderSummary}
              />
            )}
          </div>
        </section>
